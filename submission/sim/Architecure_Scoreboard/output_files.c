...
void write_traceunit_file(FILE* traceunit_file, int cycle, unit_t* trace_unit) {
    fprintf_s(traceunit_file, "%d %s %s %s %s %s %s %s %s\n",
        ...
        trace_unit->unit_id.operation == ST ? "-" : regs_str[trace_unit->Fi.old_val],
        trace_unit->unit_id.operation == ST || trace_unit->unit_id.operation == LD ? "-" : regs_str[trace_unit->Fj.old_val],
        trace_unit->unit_id.operation == LD ? "-" : regs_str[trace_unit->Fk.old_val],
